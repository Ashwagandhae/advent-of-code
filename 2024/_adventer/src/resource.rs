...
fn find_output(html: &str, index: ProblemPart) -> Result<String, FindOutputError> {
    ...
    let Some(para) = document.select(&main).nth(match index {
        ...
        return Err(FindOutputError::NoOutputYet(index));
    };
    let select = Selector::parse("code").unwrap();
    let Some(code) = para.select(&select).next() else {
        return Err(FindOutputError::NoOutputElementInPara);
    };
...
fn find_example_input(html: &str, index: ProblemPart) -> Result<String, FindExampleError> {
    fn find_example_in_article(article: ElementRef) -> Option<String> {
        ...
        iter.tuple_windows()
            .find(|(el_1, el_2)| {
                ...
                el_1.value().name() == "p"
                    && el_2.value().name() == "pre"
                    && (formatted_text.contains("for example")
                        || formatted_text.contains("consider"))
                    && formatted_text.ends_with(':')
            })
            .map(|(_, el)| el.text().join(""))
    }
    ...
    match index {
        ProblemPart::Zero => {
            let article = document
                .select(&main)
                .nth(0)
                .ok_or(FindExampleError::IndexTooHigh(index))?;
            find_example_in_article(article).ok_or(FindExampleError::NoExampleElementInArticle)
        }
        ProblemPart::One => {
            let mut articles_iter = document.select(&main).take(2);
            let (Some(article1), Some(article2)) = (articles_iter.next(), articles_iter.next())
            else {
                return Err(FindExampleError::IndexTooHigh(index));
            };
            [article2, article1]
                .into_iter()
                .find_map(find_example_in_article)
                .ok_or(FindExampleError::NoExampleElementInArticle)
        }
    }
}
...
fn find_example_output(html: &str, index: ProblemPart) -> Result<String, FindExampleError> {
    ...
    let Some(second_to_last) = paras.get(paras.len() - 2) else {
        return Err(FindExampleError::NoSecondToLastPara);
    };
    ...
    let Some(last) = codes.pop() else {
        return Err(FindExampleError::NoLastCodeElementInPara);
    };
