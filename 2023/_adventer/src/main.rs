...
use resource::{print_example_input_output, print_input, ProblemPart};
pub mod update;
use update::update;
pub mod run;
...
pub enum ResourceCommands {
    ...
    #[command(alias = "i")]
    Input(InputArgs),
    /// Print example input output for day
    #[command(alias = "e")]
    ExampleInputOutput(ExampleArgs),
}
...
pub struct WorkRunArgs {
    /// Test on input, example input/output, or both
    #[clap(value_enum, default_value_t = RunTarget::AllConditional)]
    on: RunTarget,
}
...
fn main() {
    ...
    let res = match args.command {
        ...
        Some(Commands::Resource(command)) => match command {
            ...
            ResourceCommands::ExampleInputOutput(ExampleArgs { problem }) => {
                print_example_input_output(problem_name(&problem))
            }
        },
