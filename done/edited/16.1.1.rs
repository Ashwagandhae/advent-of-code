...
impl BestSequenceFinder {
    ...
    fn get_child_rates(
        ...
        valve
            ...
            .iter()
            .map(|child| {
                self.best_sequence_rec(child.clone(), opened.clone(), minutes - 1) + pressure
            })
            .collect()
    }
    fn best_sequence_rec(&mut self, valve_index: usize, mut opened: Opened, minutes: u32) -> u32 {
        self.progress();
        // print opened
    ...
    fn best_sequence(&mut self, start_valve: usize, minutes: u32) -> u32 {
        self.best_sequence_rec(start_valve, Opened::new(), minutes + 1)
    }
